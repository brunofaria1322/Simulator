...
class Link:
    ...
    def __init__(
        ...
        self.host_id1 = host_id1
        self.host_id2 = host_id2
        self.bandwidth = bandwidth
        self.distance = distance
        self.packet_loss_rate = packet_loss_rate
        self.is_wired = is_wired

    def get_other_host_id(self, host_id: int):
        if host_id == self.host_id1:
            return self.host_id2
        elif host_id == self.host_id2:
            return self.host_id1
        else:
            raise ValueError("Host ID not in link")
    ...
    def __str__(self):
        return (
            f"Link: Host {self.host_id1} <-> Host {self.host_id2} | "
